...
class PromptBuilder
{
    ...
    private function generatePromptTemplate($media, $theme, $rules, $intentDetails, $structure)
    {
        ...
        // 書き手の立場（テーマに依存しない汎用表現）
        $template .= "【書き手の立場】\n";
        $template .= "・テーマに関する実体験を持つ書き手\n";
        $template .= "・自分の体験を淡々と共有する立場\n\n";
        ...
        // 禁止事項（テーマに依存しない汎用表現 + 媒体別）
        $template .= "【禁止事項】\n";
        $template .= "・「誰でも」「簡単に」などの煽り表現\n";
        $template .= "・過度な宣伝・収益化の話\n";
